...
void save_block_freq() {
  u16 *trace_counts = (u16 *)trace_bits;
  for (u16 i = 0; i < MAP_SIZE / sizeof(u16); ++i) {
    block_freq[i] += trace_counts[i];
  }
  if (stage_cur % stats_update_freq) return;

  char *fname = alloc_printf("%s/block_freq.json", out_dir);
  FILE *f = fopen(fname, "w");
  if (f == NULL) PFATAL("Unable to open '%s'", fname);

  fprintf(f, "{\"freq\":[%llu", block_freq[0]);
  for (u16 i = 1; i < MAP_SIZE / sizeof(u16); ++i) {
    fprintf(f, ",%llu", block_freq[i]);
  }
  fprintf(f, "]}");
